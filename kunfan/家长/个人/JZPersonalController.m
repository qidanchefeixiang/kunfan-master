...
@interface JZPersonalController ()<UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, weak) UITableView *tableView;
@property (nonatomic, strong) NSArray *homeItems;

@end

@implementation JZPersonalController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (0 == indexPath.section) {
        if (0 == indexPath.row) {
            [cell updataCell:@"消息提醒" andImage:@""];
        }else if (1 == indexPath.row){
            [cell updataCell:@"清理缓存" andImage:@""];
        }
       
    }else if(1 == indexPath.section){
        if (0 == indexPath.row) {
            [cell updataCell:@"联系我们" andImage:@""];
        }else if (1 == indexPath.row){
