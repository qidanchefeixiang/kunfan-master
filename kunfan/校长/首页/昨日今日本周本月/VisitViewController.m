...
- (void)setupLayout{
    ...
    [tableView registerClass:[VisitTableCell class] forCellReuseIdentifier:[VisitTableCell identifier]];
...
- (void)addHeaderView{
    ...
    [titleView changTitle:@"接待人数" andDetail:@"本次接待人数10人"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (0 == section) {
        ...
        self.names = [NSArray arrayWithObjects:@"接待",@"报名",nil];
...
-(void)headerBtnClick:(UIButton *)headerBtn
{
    ...
    if (currentPage == 0) {
        [self.titleView changTitle:@"接待人数" andDetail:@"本次接待人数10人"];
    }else{
        [self.titleView changTitle:@"报名人数" andDetail:@"本次报名人数10人"];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    //设置 添加好友
    VisitTableCell *cell = [tableView dequeueReusableCellWithIdentifier:[VisitTableCell identifier] forIndexPath:indexPath];
    if (!cell) {
        cell = [[VisitTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:[VisitTableCell identifier]];
    }
